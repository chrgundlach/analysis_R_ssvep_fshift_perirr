...
# exploratory | induced removed as in FShiftAlpha
# broad cluster | larger central cluster as in Tango study | peri both clusters but smaller
# CSD based normalized to pre-cue baseline in %
# for changed experiment starting with participant 22: isoluminant stimuli and lower flicker frequencies
# DATAPath = "data_in/FFT_Amp_data_CenterTangoStudyPeriSmall_indremov08-15-2025_13-50.csv"
# t_baseline = c(-1000, 0)
...
# save_multi_panel_figure(savefig, "figures/FFT_BeeSwarm_Mod_ALPHA_COL_34subs.eps")
```
### 4a  Illustrate Data as is | peri RDKs | pre-cue
...
```{r plot_data_st_peri_precue, results = "hide",  fig.height=6, fig.width=6, warning = FALSE}
...
dat2plot <- DataIn %>%
  ...
  group_by(subjects, time, RDK_isattended2, RDK_colorlum)%>%
  summarise(amplitude_evoked=mean(amplitude_evoked), modulation_evoked=mean(modulation_evoked))%>%
  filter(time=='[-1 0] ', RDK_colorlum == 'isolum__to_bckgrd')
...
theme_set(theme_bw())
plot1 <- ggplot(dat2plot, aes(x = RDK_isattended2, y = amplitude_evoked, fill = RDK_isattended2)) +
  geom_hline(yintercept=0, show.legend = FALSE) +
  geom_line(aes(group = interaction(subjects,time)),colour = "grey60",alpha = 1,size =0.5) +
  geom_beeswarm(aes(color = RDK_isattended2, x = RDK_isattended2,
                    group = RDK_isattended2), cex=2, size = 5,alpha=1,fill="grey40",shape=21)+
  ...
  scale_x_discrete("RDK_isattended2", breaks=waiver(), labels = rep(c(""),1,6)) +
  # theme(legend.position="bottom") +
  ylab(expression(paste("amplitude in muV/m²")))+
  scale_fill_brewer(palette = "Dark2")+
  scale_color_brewer(palette = "Dark2")+
  theme(legend.position="none")+
  # scale_y_continuous(limits=c(-60, 140))
...
plot2 <- ggplot(dat2plot, aes(y = amplitude_evoked, fill = RDK_isattended2, x = 1)) +
  geom_flat_violin(aes(fill = RDK_isattended2),position = position_nudge(x = .1, y = 0), 
                   adjust = 1.3, trim = FALSE, alpha = 1, colour = NA)+
  geom_hline(yintercept=0, show.legend = FALSE) +
  scale_fill_brewer(palette = "Dark2")+
...
# cowplot::plot_grid(plot1, plot2, plot3, ncol = 3,labels = "AUTO")
plotttitle <- ggdraw() + 
  draw_label("SSVEP amp. | evoked | pre-cue\nperi RDKs | isoluminant to background", fontface='bold', size = 10)
plotplots <- ggarrange(plot1, plot2,ncol = 2,common.legend = TRUE,align = "h", widths=c(3,1))
...
dat2plot <- DataIn %>%
  ...
  group_by(subjects, time, condition_label)%>%
  summarise(amplitude_evoked=mean(amplitude_evoked), modulation_evoked=mean(modulation_evoked))%>%
  filter(time == '[-1 0] ')
...
theme_set(theme_bw())
plot1 <- ggplot(dat2plot, aes(x = condition_label, y = amplitude_evoked, fill = condition_label)) +
  geom_hline(yintercept=0, show.legend = FALSE) +
  geom_line(aes(group = interaction(subjects,time)),colour = "grey60",alpha = 1,size =0.5) +
  geom_beeswarm(aes(color = condition_label, x = condition_label,
                    group = condition_label), cex=2, size = 5,alpha=1,fill="grey40",shape=21)+
  # geom_point(aes(color = condition_label, x = interaction(time,RDK_isattended),
  #                   group = condition_label), size = 4,alpha=0.4)+
  stat_summary(fun.y=mean, aes(ymin=..y.., ymax=..y.., color =condition_label), geom='errorbar', width=0.9, size=1.5) +
  scale_x_discrete("condition_label", breaks=waiver(), labels = rep(c(""),1,6)) +
  # theme(legend.position="bottom") +
  ylab(expression(paste("amplitude in muV/m²")))+
  scale_fill_brewer(palette = "Dark2")+
  scale_color_brewer(palette = "Dark2")+
...
plot2 <- ggplot(dat2plot, aes(y = amplitude_evoked, fill = condition_label, x = 1)) +
  geom_flat_violin(aes(fill = condition_label),position = position_nudge(x = .1, y = 0), 
                   adjust = 1.3, trim = FALSE, alpha = 1, colour = NA)+
  geom_hline(yintercept=0, show.legend = FALSE) +
  scale_fill_brewer(palette = "Dark2")+
  scale_color_brewer(palette = "Dark2")+
...
dat2plot %>%
  ...
  kbl(escape = F, digits = c(3,3,3,3,3,3,3,3,3), caption = c("SSVEP | modulation | center - peri ")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
...
dat2plot2 %>%
  ...
  kbl(escape = F, digits = c(3,3,3,3,3,3,3,3,3), caption = c("peripheral SSVEP | pre-cue amplitudes | contrasts")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
