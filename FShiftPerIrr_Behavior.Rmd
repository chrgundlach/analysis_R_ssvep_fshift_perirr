...
```{r plot_data_avg2, results = "hide",  fig.height=6, fig.width=10, warning = FALSE}
...
dat2plot1a = DataIn1%>%
  ...
  dplyr::summarise(
    ...
    FArate_adj2 = (sum(response=='FA_proper')+0.5)/(n()+1)) %>% # loglinear approach (Hautus, 1995))
  ungroup()
dat2plot <- merge(dat2plot1,dat2plot1a,by = c("participant","colorlum")) %>% # adjust for HR and FR of 0 or 1
  mutate(
    d_prime = qnorm(Hitrate_adj1)-qnorm(FArate_adj1),
    c = -0.5*(qnorm(Hitrate_adj1)+qnorm(FArate_adj1))
  )
...
plot1 <- ggplot(dat2plot, aes(x = colorlum, y = RT_Mean, color =colorlum,)) +
  # geom_hline(yintercept=0, show.legend = FALSE) +
  geom_beeswarm(cex=4.5, size = 3,alpha=0.4)+
  stat_summary(fun.y=mean, aes(ymin=..y.., ymax=..y..), geom='errorbar', width=0.9, size=1.5) +
  scale_fill_brewer(palette = "Dark2")+
  scale_color_brewer(palette = "Dark2")+
  # scale_fill_manual("attended color", values=c("darkcyan", "darkgreen", "darkorange")) +
...
plot3 <- ggplot(dat2plot, aes(x = colorlum, y = d_prime, color =colorlum)) +
  # geom_hline(yintercept=0, show.legend = FALSE) +
  geom_beeswarm(cex=4.5, size = 3,alpha=0.4)+
  stat_summary(fun.y=mean, aes(ymin=..y.., ymax=..y..), geom='errorbar', width=0.9, size=1.5) +
  scale_fill_brewer(palette = "Dark2")+
...
plot3b <- ggplot(dat2plot, aes(x = 1, y = d_prime, color =colorlum, fill = colorlum)) +
  geom_flat_violin(position = position_nudge(x = .1, y = 0), adjust = 1.0, trim = FALSE, alpha = 0.8)+
  # scale_fill_manual(values=c("grey40"))+
  scale_fill_brewer(palette = "Dark2")+
  # scale_color_manual(values=c("grey40"))+
...
plot4 <- ggplot(dat2plot, aes(x = colorlum, y = FArate_adj1, color =colorlum)) +
  # geom_hline(yintercept=0, show.legend = FALSE) +
  geom_beeswarm(cex=4.5, size = 3,alpha=0.4)+
  stat_summary(fun.y=mean, aes(ymin=..y.., ymax=..y..), geom='errorbar', width=0.9, size=1.5) +
  scale_fill_brewer(palette = "Dark2")+
  ...
  # scale_fill_manual("attended color", values=c("darkcyan", "darkgreen", "darkorange")) +
  # scale_color_manual("attended color", values=c("darkcyan", "darkgreen", "darkorange"))
  ylab(expression(paste("FA rate in %")))+
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())+
  scale_y_continuous(limits=c(0.0, 0.5))

plot4b <- ggplot(dat2plot, aes(x = 1, y = FArate_adj1, color =colorlum, fill = colorlum)) +
  geom_flat_violin(position = position_nudge(x = .1, y = 0), adjust = 1.0, trim = FALSE, alpha = 0.8)+
  # scale_fill_manual(values=c("grey40"))+
  scale_fill_brewer(palette = "Dark2")+
  # scale_color_manual(values=c("grey40"))+
...
plot5 <- ggplot(dat2plot, aes(x = colorlum, y = c, color =colorlum)) +
  # geom_hline(yintercept=0, show.legend = FALSE) +
  geom_beeswarm(cex=4.5, size = 3,alpha=0.4)+
  stat_summary(fun.y=mean, aes(ymin=..y.., ymax=..y..), geom='errorbar', width=0.9, size=1.5) +
  scale_fill_brewer(palette = "Dark2")+
...
plot5b <- ggplot(dat2plot, aes(x = 1, y = c, color =colorlum, fill = colorlum)) +
  geom_flat_violin(position = position_nudge(x = .1, y = 0), adjust = 1.0, trim = FALSE, alpha = 0.8)+
  # scale_fill_manual(values=c("grey40"))+
  scale_fill_brewer(palette = "Dark2")+
  # scale_color_manual(values=c("grey40"))+
...
# cowplot::plot_grid(plot1, plot2, plot3, ncol = 3,labels = "AUTO")
plotttitle <- ggdraw() + draw_label("behavioral measures", fontface='bold')
plotplots <- ggarrange(plot1, plot1b,plot2, plot2b,plot4, plot4b,plot3, plot3b,plot5, plot5b, 
                       ncol = 10,common.legend = TRUE,align = "h", widths=c(3,2,3,2,3,2,3,2,3,2))

savefig <- multi_panel_figure(columns = 1, rows = 1, panel_label_type = "none", width = 10, height = 6, unit = "in")
saveplot <- ggarrange(plotttitle, plotplots, nrow = 2, heights=c(0.2, 1))
...
dat2plot  %>%
  group_by(colorlum)%>%
  dplyr::summarise(N = n(), M_RT = mean(RT_Mean), SD_RT = sd(RT_Mean), M_HR = mean(Hitrate), SD_HR = sd(Hitrate),
            M_FAR = mean(FArate), SD_FAR = sd(FArate), M_dprime = mean(d_prime), SD_dprime = sd(d_prime),
            M_c = mean(c), SD_c = sd(c)) %>%
...
dat2plot  %>%
  dplyr::summarise(N = n(), M_RT = mean(RT_Mean), SD_RT = sd(RT_Mean), M_HR = mean(Hitrate), SD_HR = sd(Hitrate),
            M_FAR = mean(FArate), SD_FAR = sd(FArate), M_dprime = mean(d_prime), SD_dprime = sd(d_prime),
            M_c = mean(c), SD_c = sd(c)) %>%
  kable(escape = F, digits = c(4,4,4,4,4,4,4,4,4,4), caption = c("descriptives of behavioral measures")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
...
dat2plot_all %>%
  nest() %>%
  mutate(
    stats = map(data, ~ broom::tidy(t.test(RT_Mean ~ colorlum, data = .))),
...
dat2plot_all %>%
  nest() %>%
  mutate(
    stats = map(data, ~ broom::tidy(t.test(Hitrate ~ colorlum, data = .))),
...
dat2plot_all %>%
  nest() %>%
  mutate(
    stats = map(data, ~ broom::tidy(t.test(FA_proper_rate ~ colorlum, data = .))),
...
dat2plot_all %>%
  nest() %>%
  mutate(
    stats = map(data, ~ broom::tidy(t.test(d_prime ~ colorlum, data = .))),
...
dat2plot_all %>%
  nest() %>%
  mutate(
    stats = map(data, ~ broom::tidy(t.test(c ~ colorlum, data = .))),
...
#### test RT, Hitrate and dprime between different conditions of stimuli in periphery

**RT** ~ **conditions**

<br> 

```{r lme_RT_lum_con, fig.show = "hide", warning = FALSE, fig.height=4, fig.width=10}
dat2plot <-  DataIn1%>%
  # filter(eventtype=="target") %>%
  mutate(condition2 = case_when(
    condition %in% c(1,2) ~ "Att+Unatt",
  ...
  filter(colorlum=="isolum__to_bckgrd")%>%
  group_by(participant,condition2,eventtype)%>%
  dplyr::summarise(
    RT_Mean  = mean(RT, na.rm = TRUE),
    RT_STD   = sd(RT, na.rm = TRUE),
    Hitrate  = ifelse(first(eventtype) == "target",
                      (sum(response == 'hit') / n()) * 100, NA),
    FArate   = ifelse(first(eventtype) == "distractor",
                      (sum(response == 'FA_proper') / n()) * 100, NA),
    .groups = "drop"
  )
...
dat2plot%>%
  filter(eventtype=="target")%>%
  aov_ez(id="participant", dv = "RT_Mean", data = ., within = c("condition2"),
  ...
  kbl(escape = F, digits = c(5,5,5,5,5,5,5,5,5), caption = c("ANOVARM  for RT")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
...
RTmodel <- dat2plot %>%
  filter(eventtype=="target")%>%
  mixed(
    RT_Mean ~ condition2 + (1 | participant),
    data = .,
...
eta_tab <- effectsize::eta_squared(RTmodel, generalized = TRUE, partial = FALSE) %>%
  as.data.frame() %>%
...
RTmodel %>%
  ...
  kbl(escape = F, digits = c(5,5,5,5,5,5,5,5,5), 
      caption = c("Mixed Model Anova Table (Type 3 tests, KR-method) | RT model")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
...
dat2plot%>%
  filter(eventtype=="target")%>%
  aov_ez(id="participant", dv = "Hitrate", data = ., within = c("condition2"),
         include_aov = afex_options("include_aov")) %>%
  anova()%>%
  tidy() %>%
   mutate(
  ...
  dplyr::select(term, `num.Df`, `den.Df`, MSE, `statistic`, ges, `p.value`) %>%
  dplyr::rename(F=statistic)%>%
  kbl(escape = F, digits = c(5,5,5,5,5,5,5,5,5), caption = c("ANOVARM  for Hitrate")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
...
dat2plot%>%
  filter(eventtype=="distractor")%>%
  aov_ez(id="participant", dv = "FArate", data = ., within = c("condition2"),
         include_aov = afex_options("include_aov")) %>%
  anova()%>%
  tidy() %>%
   mutate(
  ...
  dplyr::select(term, `num.Df`, `den.Df`, MSE, `statistic`, ges, `p.value`) %>%
  dplyr::rename(F=statistic)%>%
  kbl(escape = F, digits = c(5,5,5,5,5,5,5,5,5), caption = c("ANOVARM  for FA rate")) %>%
  kable_styling("striped", full_width = T, bootstrap_options = c("striped", "hover", "condensed", "responsive",font_size = 8))
