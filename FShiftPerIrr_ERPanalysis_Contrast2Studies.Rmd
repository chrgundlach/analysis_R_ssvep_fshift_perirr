...
DATAPath = c("data_in/ERP_N2_ParietemporalLateral_230to430ms.csv",
              "data_in/ERP_P300_centroparietal_450to650ms.csv")
...
### 0  Count trial number

<br>  

```{r trialnumber, results = "hide",  fig.height=5, fig.width=5.5, warning = FALSE}
DataIn %>%
  ungroup() %>%
  group_by(luminance, eventtype)%>%
  dplyr::summarise(M = mean(trialnum), SD = sd(trialnum)) %>%
  kbl(escape = F, digits = c(2,2,2,2), caption = c("number of trials per condition")) %>%
...
```{r plot_data_ERP_N2, results = "hide",  fig.height=3, fig.width=3, warning = FALSE}
...
yax_limits <- rev(yax_limits + c(-3.5, 1)*diff(yax_limits)*0.1)
...
plot1 <- ggplot(dat2plot, aes(x = interaction(eventtype,luminance), y = amplitude, fill = eventtype)) +
  ...
  geom_beeswarm(aes(color = eventtype, x = interaction(eventtype,luminance),
                    group = eventtype), cex=2, size = 3,alpha=1,fill="grey60",shape=21)+
  ...
  scale_y_continuous(limits=yax_limits, transform = "reverse")


plot2 <- ggplot(dat2plot, aes(y = amplitude, fill = eventtype, x = luminance)) +
  geom_flat_violin(aes(fill = eventtype, width = 1.5),
  ...
  # scale_y_continuous(limits=c(-60, 140))
  scale_y_continuous(limits=rev(yax_limits), transform = "reverse")
...
savefig <- multi_panel_figure(columns = 1, rows = 1, panel_label_type = "none", width = 3, height = 3, unit = "in")
saveplot <- ggarrange(plotttitle, plotplots, nrow = 2, heights=c(0.2, 1))

savefig %<>%
  fill_panel(saveplot, column = 1, row = 1)
savefig
# save_multi_panel_figure(savefig, "figures/ERP_N2.eps")
...
```{r plot_data_ERP_P300, results = "hide",  fig.height=3, fig.width=3, warning = FALSE}
#evoked raw#### for center
...
yax_limits <- c( min(dat2plot$amplitude), max(dat2plot$amplitude))
yax_limits <- round(yax_limits + c(-1, 1)*diff(yax_limits)*0.1)
yax_limits <- yax_limits + c(-3, 1)*diff(yax_limits)*0.1
...
savefig %<>%
  fill_panel(saveplot, column = 1, row = 1)
savefig
# save_multi_panel_figure(savefig, "figures/ERP_P300.eps")
...
do differences between target/distractor N2 correlate with P3 differences?
...
# high luminance experiment
dat2plot <- DataIn %>%
  dplyr::select(-time_window, -electrodes, -trialnum)%>%
  pivot_wider(names_from = c("erpcomponent","eventtype"), values_from = "amplitude") %>%
  mutate("N2diff" = N2_target- N2_distractor, "P300diff" = P300_target- P300_distractor)%>%
  filter(luminance=="offset_to_bckgrd")

corrRho1 = SpearmanRho(x = dat2plot$N2diff, y = dat2plot$P300diff, conf.level = .95)
cbf1 = correlationBF(dat2plot$N2diff, dat2plot$P300diff, rscale = "medium",iterations=num.iter)
plot1<-ggscatter(dat2plot, x = "N2diff", y = "P300diff", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = def.cormethod, size = 3, alpha = 0.6,
          add.params = list(fill = "lightgray"),
          ggtheme = theme_classic(),
          xlab = "N2", ylab = "P300",
          title = sprintf("Rho: %1.3f CI[%1.3f %1.3f]; BF10: %1.3f",corrRho1[[1]],corrRho1[[2]],corrRho1[[3]],extractBF(cbf1)[[1]]))+
  font("title", size = 8)



plotttitle <- ggdraw() + draw_label("Spearman Rank Correlations\nhigh luminance experiment", fontface='bold', size=12)
ggarrange(plotttitle, plot1,nrow = 2, heights=c(0.2, 1))

# isoluminance experiment
dat2plot <- DataIn %>%
  dplyr::select(-time_window, -electrodes, -trialnum)%>%
  pivot_wider(names_from = c("erpcomponent","eventtype"), values_from = "amplitude") %>%
  mutate("N2diff" = N2_target- N2_distractor, "P300diff" = P300_target- P300_distractor)%>%
  filter(luminance=="isolum__to_bckgrd")
...
plotttitle <- ggdraw() + draw_label("Spearman Rank Correlations\nisoluminance experiment", fontface='bold', size=12)
ggarrange(plotttitle, plot1,nrow = 2, heights=c(0.2, 1))
